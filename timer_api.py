...
# API route to handle timer data and store in MySQL
...
def timer():
    ...
    if conn:
        try:
            # Insert timer data into the 'sessions' table
            cursor = conn.cursor()
            insert_query = """
            ...
            return jsonify({"message": "Session data stored in the database successfully!"}), 201
        except Error as e:
